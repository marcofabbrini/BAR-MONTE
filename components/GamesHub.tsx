...
import { TombolaConfig, AnalottoConfig, LotteryConfig } from '../types';
import { BackArrowIcon, GamepadIcon, CloverIcon, DiceIcon, BoxIcon, TicketIcon } from './Icons';
...
const GamesHub: React.FC<GamesHubProps> = ({ onGoBack, onPlayTombola, onPlayAnalotto, onPlayDice, onOpen3DViewer, onPlayLottery, tombolaConfig, analottoConfig, lotteryConfig }) => {
    return (
            ...
            <div className="flex-grow p-4 md:p-8 max-w-7xl mx-auto w-full animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {lotteryConfig?.isActive && (
                        <div onClick={onPlayLottery} className="relative bg-white rounded-3xl shadow-xl overflow-hidden cursor-pointer transform hover:scale-105 transition-all duration-300 group border-2 border-green-100 h-64 flex flex-col items-center justify-center">
                            <div className="absolute top-0 left-0 w-full h-2 bg-green-500"></div>
                            <div className="text-[70px] leading-none mb-2 filter drop-shadow-lg group-hover:scale-110 transition-transform">
                                🎟️
                            </div>
                            <h3 className="text-2xl font-black text-slate-800 uppercase tracking-wider mb-1">Lotteria</h3>
                            <p className="text-slate-400 text-xs font-bold mb-4 uppercase tracking-widest">Estrazione: {lotteryConfig.extractionDate ? new Date(lotteryConfig.extractionDate).toLocaleDateString() : 'TBD'}</p>
                            
                            <div className="bg-green-50 px-4 py-2 rounded-full border border-green-100 text-green-500 font-bold text-xs uppercase">
                                Biglietti al Bar
                            </div>
                        </div>
                    )}
